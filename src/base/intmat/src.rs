...
pub type IntMatSpace = MatSpace<IntegerRing>;
...
impl Parent for IntMatSpace {
    ...
    fn default(&self) -> IntMat {
        IntMat::zero(self.nrows, self.ncols)
    }
}
...
impl Multiplicative for IntMatSpace {
    #[inline]
    fn one(&self) -> IntMat {
        IntMat::one(self.nrows, self.ncols)
    }
}
...
impl<T> InitParent2<T, T> for IntMatSpace where 
    T: TryInto<c_long>,
{
    #[inline]
...
impl NewElement<&IntMat> for IntMatSpace {
    fn new(&self, x: &IntMat) -> IntMat {
        x.clone()
    }
}

impl<T> NewElement<T> for IntMatSpace where 
    T: Into<IntMat>
{
    #[inline]
